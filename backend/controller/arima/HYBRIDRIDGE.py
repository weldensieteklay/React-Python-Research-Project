...
def predict_price_hybrid_ridge():
    try:
        ...
        ridge_pipeline.fit(X_train, y_ridge)

        resid_preds = ridge_pipeline.predict(X_test)
        ...
        return jsonify({
            "error": "Hybrid SARIMAX + Ridge failed",
            "details": repr(e)
        }), 500
