...
def predict_price_hybrid_boosting():
    try:
        ...
        gb.fit(X_train, y_boost)

        resid_preds = gb.predict(X_test)
        ...
        importance = [
            ...
            for name, val in zip(X_train.columns, gb.feature_importances_)
        ]

        return jsonify(to_serializable({
            "model": "sarimax_gradient_boosting_hybrid",
            "rmse": round(rmse, 3),
            "mae": round(mae, 3),
            "mse": round(mse, 3),
        ...
        return jsonify({
            "error": "Hybrid SARIMAX + Gradient Boosting failed",
            "details": repr(e)
        }), 500
