...
# separate from OOB scoring, which is the primary/near-free CV-equivalent
# for Random Forest (same mechanism as Bagging: each tree is fit on a
# bootstrap sample, leaving ~37% of rows unused per tree that can be
# scored against without extra fitting).
CV_FOLDS = 5
CV_N_ESTIMATORS = 100  # lighter cap during the CV loop to control runtime
...
def build_feature_importances(model, columns, include_std=False):
    ...
    if include_std:
        ...
        importance_std = {
            ...
            for k, v in zip(columns, tree_importances.std(axis=0))
        }
    return importances, ranked, importance_std
...
def _run_continuous(X_train, X_test, y_train, y_test):
    """Random Forest regression path, with OOB score added as a free CV-equivalent."""
    ...
    importances, ranked, _ = build_feature_importances(model, X_train.columns)
    ...
    return {
        ...
        # that did NOT see that row during bootstrap sampling. Near-free
        # cross-validation-style estimate from the single fit above.
        "oob_r2_score": oob_score,
        "feature_importances":        importances,   # plural, for cross-model consistency
        "feature_importance":         importances,   # singular, kept for backward compatibility
...
def _run_binary(X_train, X_test, y_train, y_test, threshold=0.5):
    """
    Random Forest classifier for binary (0/1) targets, with OOB accuracy
    added as a free CV-equivalent. No scaling needed — tree models are
    scale-invariant.
    """
    ...
    importances, ranked, importance_std = build_feature_importances(
        model, X_train.columns, include_std=True
    )
...
def run_kfold_cv_continuous(X, y, k=CV_FOLDS, random_state=RANDOM_STATE):
    ...
    a (lighter, CV_N_ESTIMATORS-tree) Random Forest across k different
    train/validation partitions of the full dataset and reports the
    spread of out-of-sample R^2/RMSE/MAE.
    """
    min_rows_needed = k * 5
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(kf.split(X_reset), start=1):
        ...
        try:
            fold_model = RandomForestRegressor(
                n_estimators=CV_N_ESTIMATORS, random_state=random_state, n_jobs=-1,
            )
    ...
    return {
        ...
        "note": (
            ...
            f"Compare r2_mean/r2_std here and oob_r2_score above against the "
            f"single-split r2_score to gauge performance stability."
        ),
    }
