...
def _run_binary(X_train, X_test, y_train, y_test, threshold=0.5):
    """L1 logistic regression path for binary (0/1) targets."""
    ...
    model = LogisticRegression(
        penalty="l1",
        solver="saga",
        C=1.0,          # C = 1/alpha, mirrors alpha=1.0 in the continuous path
        max_iter=10000,
        random_state=42,
    )
    ...
    return {
        ...
        # Kept None for API consistency with the continuous path
        "standard_errors":          {str(k): None for k in X_train.columns},
        "p_values":                 {str(k): None for k in X_train.columns},
        "selected_features":        selected,
        "eliminated_features":      eliminated,
...
async def run_lasso_cross_sectional_prediction(request):
    try:
        ...
        # ── Train / test split ──
        # Stratify when binary to preserve class ratio in both sets
