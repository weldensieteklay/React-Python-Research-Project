...
def _fit_ridge_fold(train_df, dependent_col, X_cols, id_col, alpha):
    """
    Fit Ridge on ONE fold's training rows only: entity means used for
    demeaning are computed from train_df alone (not the full dataset),
    so no information about held-out rows leaks into how training rows
    get demeaned. Also recovers each training entity's fixed effect
    (alpha_i = mean(y_i) - mean(X_i) @ beta) -- the correct FE
    definition, not just the raw mean of y -- so held-out rows for
    that entity can be predicted on the original (level) scale.
    """
...
def _predict_ridge_fold(test_df, dependent_col, id_col, fold_fit):
...
def _run_ridge_cv_fold(train_df, test_df, dependent_col, X_cols, id_col, alpha):
    if test_df.empty:
        return None
    fold_fit = _fit_ridge_fold(train_df, dependent_col, X_cols, id_col, alpha)
    if fold_fit is None:
        return None
    y_true, y_pred = _predict_ridge_fold(test_df, dependent_col, id_col, fold_fit)
    if y_true is None:
        return None
...
def run_ridge_cross_validation(df, dependent_col, X_cols, id_col, time_col, alpha, cv_folds=3):
    """
    Same reasoning as the Fixed Effects endpoint: Ridge-on-demeaned-
    data implicitly estimates an entity fixed effect, so it can't
    meaningfully predict for an entity it never trained on -- time-
    based walk-forward CV (train on earlier periods, predict later
    periods for the SAME entities) is preferred when a usable
    date_column exists. Entity-based GroupKFold is the fallback,
    using the average fixed effect for held-out entities as a
    necessarily weaker stand-in.
    """
    fold_metrics = []
    method = None
    ...
    if time_col and df[time_col].nunique() >= cv_folds + 1:
        ...
        for train_t_idx, test_t_idx in splitter.split(unique_times):
            ...
            result = _run_ridge_cv_fold(train_df, test_df, dependent_col, X_cols, id_col, alpha)
            if result:
...
async def run_ridge_panel(request):
    try:
        ...
        cross_validation = run_ridge_cross_validation(
        ...
        # ── Fit Ridge on the full within-transformed sample ──
        ...
        # In-sample within-R² (same scale as the original endpoint's metric)
        ...
        coefficients = {str(name): safe_round(c, 6) for name, c in zip(coef_names, model.coef_)}
        ...
        # (the original computed only mean(y_i), which ignores the
        # regressors entirely and isn't a real fixed-effect estimate)
